  ...
  async addOrEdit() {
    ...
    if (this.seleccionado.id === 0) { // nuevo
      ...
      try {
        ...
        const index = this.plainStock.findIndex(x => x.id === this.seleccionado.id);
        if (index !== -1) {
          this.plainStock[index] = this.seleccionado;
        }
        
        this.seleccionadoBackup = new PlainStock(this.seleccionado);
        this.unselect();
      }
      catch (error) { this.messages = error; }
      finally { this.loading = false; }
